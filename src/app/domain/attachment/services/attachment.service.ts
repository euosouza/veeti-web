...
export class AttachmentService {
  private api = inject(AttachmentApi);
  ...
  loadByPetId(petId: string) {
    ...
    this.api
      .getByPetId(petId)
      .pipe(
        tap((data) => this.attachments.set(data)),
        finalize(() => this.loading.set(false))
      )
      .subscribe();
  }

  create(payload: ICreateAttachment) {
    this.loading.set(true);
    return this.api.create(payload).pipe(
      tap((newItem) => this.attachments.update((list) => [...list, newItem])),
      finalize(() => this.loading.set(false))
    );
  }

  update(id: string, payload: IUpdateAttachment) {
    this.loading.set(true);
    return this.api.update(id, payload).pipe(
      tap((updated) => this.attachments.update((list) => list.map((a) => (a.id === id ? updated : a)))),
      finalize(() => this.loading.set(false))
    );
  }
  ...
  delete(id: string) {
    ...
    return this.api.delete(id).pipe(
      tap(() => this.attachments.update((list) => list.filter((a) => a.id !== id))),
      finalize(() => this.loading.set(false))
    );
  }
}
