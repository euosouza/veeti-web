import { Injectable, computed, inject, signal } from "@angular/core";
import { finalize, tap } from "rxjs/operators";
import { QueryOptions } from "src/app/shared/interfaces/query-options.interface";
import { PrescriptionApi } from "../apis/prescription.api";
import { ICreatePrescription, IPrescription, IUpdatePrescription } from "../interfaces/prescription.interface";

@Injectable({
  providedIn: "root"
})
export class PrescriptionService {
  private api = inject(PrescriptionApi);

  // State
  prescriptions = signal<IPrescription[]>([]);
  currentPrescription = signal<IPrescription | null>(null);
  loading = signal<boolean>(false);
  private totalItemsSignal = signal<number>(0);
  private pageSignal = signal<number>(1);
  private pageSizeSignal = signal<number>(10);

  // Selectors
  public totalItems = computed(() => this.totalItemsSignal());
  public page = computed(() => this.pageSignal());
  public pageSize = computed(() => this.pageSizeSignal());

  loadAll(query?: string) {
    this.loading.set(true);

    const options: QueryOptions = {
      page: this.pageSignal(),
      limit: this.pageSizeSignal(),
      search: query
    };

    return this.api
      .getAll(options)
      .pipe(
        tap((response) => {
          this.prescriptions.set(response.data ?? []);
          this.totalItemsSignal.set(response.meta.total ?? 0);
        }),
        finalize(() => this.loading.set(false))
      )
      .subscribe();
  }

  setPage(page: number) {
    this.pageSignal.set(page);
  }

  setPageSize(size: number) {
    this.pageSizeSignal.set(size);
    this.pageSignal.set(1);
  }

  loadByPetId(petId: string) {
    this.loading.set(true);
    this.api
      .getByPetId(petId)
      .pipe(
        tap((data) => this.prescriptions.set(data)),
        finalize(() => this.loading.set(false))
      )
      .subscribe();
  }

  loadById(id: string) {
    this.loading.set(true);
    this.api
      .getById(id)
      .pipe(
        tap((data) => this.currentPrescription.set(data)),
        finalize(() => this.loading.set(false))
      )
      .subscribe();
  }

  create(payload: ICreatePrescription) {
    this.loading.set(true);
    return this.api.create(payload).pipe(
      tap((newItem) => this.prescriptions.update((list) => [...list, newItem])),
      finalize(() => this.loading.set(false))
    );
  }

  update(id: string, payload: IUpdatePrescription) {
    this.loading.set(true);
    return this.api.update(id, payload).pipe(
      tap((updated) => this.prescriptions.update((list) => list.map((p) => (p.id === id ? updated : p)))),
      finalize(() => this.loading.set(false))
    );
  }

  delete(id: string) {
    this.loading.set(true);
    return this.api.delete(id).pipe(
      tap(() => this.prescriptions.update((list) => list.filter((p) => p.id !== id))),
      finalize(() => this.loading.set(false))
    );
  }
}
